...
/*
	$.ajax()
		...
		별도의 인자가 존재하지 않고 JSON형태의 인자 하나만 사용된다.
	속성 
	url : 요청할 서버의 경로
	...
	data : 서버로 전송 할 파라미터. JSON형식
*/

$(function(){
	
	/*
	해당 메소드는 문서의 로드가 끝난직 후 js파일의 내용을 읽어와서
	현재 문서에 로드한다. 즉$.load()메소드와 동일한 역할을 하고있다.
	*/
	$.ajax({
		url : './common/04JsData.js',
		type : 'get', //전송방식
		dataType : "script", //콜백해주는 데이터타입
	...
	$('#ajaxBtn').click(function(){
		$.ajax({
			...
			success : sucFunc, /* sunFunc(data) 형태로 사용하지 않는다.*/
			error : errFunc
		});
	});
});
...
/*
 	요청 성공시 호출한 콜백메소드. 외부함수로 정의되어 있고
